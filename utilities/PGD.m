function [A_star, flag, obj, res] = PGD(mu_x, cov_x, rho_x, mu_w, cov_w, rho_w, H, opts)
% Projected gradient descent algorithm for Solving problem (17)
%
% Syntax: [A_star, b_star, obj] = PGD(mu_x, cov_x, rho_x, mu_w, cov_w, rho_w, H)
%
% Long description
    ...
    if verbose
        fprintf('%s\n', repmat('*', 1, 22));
        fprintf(strcat(step_size, ' PGD''s method \n'));
        fprintf('%s\n', repmat('*', 1, 22));
...
%                 eta_k = min(g_k/(beta * P_norm), 1);
            case 'Armijo'
                ...
                while true
                    eta_k = beta_k;
                    eta_w = eta_k;
                    tmp_x = eye(n) + eta_k * D_x;
                    Sigma_x_tmp_1_2 = tmp_x * var_cov_x * tmp_x;
                    tmp_w = eye(n) + eta_w * D_w;
                    Sigma_w_tmp_1_2 = tmp_w * var_cov_w * tmp_w;
                    [var_cov_x_tmp, ~] = projected_wasserstein(Sigma_x_tmp_1_2, cov_x, rho_x);
                    [var_cov_w_tmp, ~] = projected_wasserstein(Sigma_w_tmp_1_2, cov_w, rho_w);
                    cov_x_half = sqrtm(var_cov_x);
                    cov_w_half = sqrtm(var_cov_w);
                    tmp_val_x = trace(inv(cov_x_half)*D_x*cov_x_half*sqrtm(cov_x_half * var_cov_x_tmp * cov_x_half))-trace(D_x*var_cov_x);
                    tmp_val_w = trace(inv(cov_w_half)*D_w*cov_w_half*sqrtm(cov_w_half * var_cov_w_tmp * cov_w_half))-trace(D_w*var_cov_w);
                    if f(var_cov_x_tmp, var_cov_w_tmp) >= ...
                            f(var_cov_x, var_cov_w) + 0.5 * (tmp_val_x + tmp_val_w)
                        break
                    end
                    beta_k = beta_k / tau;
                end
...
%             eta_k = eta_k * 0.95;
%         end
%         eta_k = 1 / norm(D_x, "fro");
%         eta_w = 1 / norm(D_w, "fro");
        ...
        switch strategy
            case 'BWGD'
                tmp_x = eye(n) + eta_k * D_x;
                Sigma_x_1_2 = tmp_x * var_cov_x * tmp_x;
                tmp_w = eye(m) + eta_w * D_w;
                Sigma_w_1_2 = tmp_w * var_cov_w * tmp_w; 
            case 'EGD'
                Sigma_x_1_2 = var_cov_x + eta_k * D_x;
                Sigma_w_1_2 = var_cov_w + eta_w * D_w;
        end
        
        % Updating variables
        if rho_x > 0
            [var_cov_x, ~] = projected_wasserstein(Sigma_x_1_2, cov_x, rho_x);
        end
        if rho_w > 0
            [var_cov_w, ~] = projected_wasserstein(Sigma_w_1_2, cov_w, rho_w);
        end
%         fprintf('The wasserstein projection flag: %d for x %d for w \n', flag_x, flag_w);
    end
